 ...
 *
 * Any leaf node (i.e., node with only one edge) can never give us an MHT
 * because its adjacent non-leaf nodes will always give an MHT with a smaller height.
 * Once we remove the leaves, we will have new leaves. Since these new leaves can’t give us MHT,
 * we will repeat the process and remove them from the graph too.
